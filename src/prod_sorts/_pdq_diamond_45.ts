/**
 * Diamond-Polished PDQSort v4.5 (TypeScript Variant)
...
function mergeRuns(A: Float64Array, lo: number, mid: number, hi: number, buffer: Float64Array) {
...
function tryVergesort(A: Float64Array, start: number, end: number): boolean {
    ...
    let buffer = new Float64Array((n >> 1) + 1);

    if (runs.length === 2) {
        const mid = runs[1].start;
        const leftLen = mid - start;
        if (leftLen > buffer.length) buffer = new Float64Array(leftLen); // Resize fix
